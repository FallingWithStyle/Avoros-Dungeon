...
export interface IStorage {
  ...
  resetUserPrimarySponsorshipForNewSeason(
    userId: string,
    seasonNumber: number,
  ): Promise<User>;
  ...
  getMarketplaceListings(
    limit?: number,
  ): Promise<MarketplaceListingWithDetails[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  private async assignRandomStartingEquipment(
    crawlerId: number,
  ): Promise<void> {
    ...
    const basicEquipment = availableEquipment.filter(
      (eq) => eq.rarity === "common",
    );

    for (let i = 0; i < numItems; i++) {
      const randomEquipment =
        basicEquipment[Math.floor(Math.random() * basicEquipment.length)];
      if (randomEquipment) {
  ...
  async getCrawlersBySponssor(
    sponsorId: string,
  ): Promise<CrawlerWithDetails[]> {
  ...
  async getCrawlerClasses(): Promise<CrawlerClass[]> {
    return await db
      .select()
      .from(crawlerClasses)
      .orderBy(asc(crawlerClasses.name));
  }
  ...
  async canCreatePrimaryCrawler(userId: string): Promise<boolean> {
    ...
    if (
      user.primarySponsorshipUsed &&
      user.lastPrimarySponsorshipSeason === currentSeason.seasonNumber
    ) {
      return false;
    }
  ...
  async getAvailableSecondarySponsorships(): Promise<CrawlerWithDetails[]> {
    ...
    const availableCrawlers = await db
      ...
      .leftJoin(crawlerClasses, eq(crawlers.classId, crawlerClasses.id))
      .where(
        and(eq(crawlers.isAlive, true), eq(crawlers.status, "seeking_sponsor")),
      );
  ...
  async resetUserPrimarySponsorshipForNewSeason(
    userId: string,
    seasonNumber: number,
  ): Promise<User> {
    const [user] = await db
  ...
  async exploreFloor(crawlerId: number): Promise<any> {
    ...
    if (!crawler || crawler.energy < 10 || !crawler.isAlive) {
      throw new Error(
        "Cannot explore - insufficient energy or crawler is dead",
      );
    }
  ...
  applyCompetencyBonus(crawler: any, encounterType: string): number {
    ...
    const relevantCompetencies =
      (competencyBonuses as any)[encounterType] || [];
  ...
  private async applyCrawlerUpdates(
    crawlerId: number,
    rewards: any,
  ): Promise<void> {
    const crawler = await this.getCrawler(crawlerId);
        ...
        if (equipmentToRemove.length > 0) {
          await db
            ...
            .where(
              eq(
                crawlerEquipment.id,
                equipmentToRemove[0].crawler_equipment.id,
              ),
            );
        }
      }
  ...
  async regenerateEnergy(crawlerId: number): Promise<Crawler> {
    ...
    const lastRegen = new Date(
      (crawler.lastEnergyRegen || crawler.createdAt) as string | Date,
    );
    ...
    const energyToRestore = Math.min(
      minutesPassed,
      crawler.maxEnergy - crawler.energy,
    );
  ...
  async generateCrawlerCandidates(count = 30): Promise<any[]> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const topAbility = statValues.reduce((max, stat) =>
  ...
  async processEncounterChoice(
    ...
    const meetsRequirements = this.checkChoiceRequirements(
      crawler,
      choice.requirements,
    );

    // Calculate success based on stats and competencies
    const successChance = this.calculateChoiceSuccess(
      crawler,
      choice,
      meetsRequirements,
    );
    ...
    const results = this.generateChoiceResults(
      crawler,
      choice,
      encounterData.type,
      isSuccess,
  ...
  private calculateChoiceSuccess(
    crawler: any,
    choice: any,
    ...
    // Competency bonus
    const competencyBonus = this.applyCompetencyBonus(
      crawler,
      choice.primaryStat,
    );
  ...
  private generateChoiceResults(
    ...
    if (!isSuccess) {
      ...
      results.damage = Math.floor(
        (10 + Math.random() * 15) * baseFloorMultiplier,
      );

      // Trading failures might still lose items
      if (
        choice.text.toLowerCase().includes("trade") &&
        crawler.equipment?.length > 0
      ) {
        const randomItem =
          crawler.equipment[
            Math.floor(Math.random() * crawler.equipment.length)
          ];
    ...
    if (
      choice.text.toLowerCase().includes("trade") ||
      choice.text.toLowerCase().includes("offer")
    ) {
    ...
    if (results.itemsLost.length > 0)
      summaryParts.push(`lost ${results.itemsLost.join(", ")}`);
    if (results.itemsGained.length > 0)
      summaryParts.push(`gained ${results.itemsGained.join(", ")}`);
  ...
  private getSuccessXP(choice: any, encounterType: string): number {
    ...
    if (
      choice.text.toLowerCase().includes("run") ||
      choice.text.toLowerCase().includes("flee")
    ) {
  ...
  private generateOutcomeMessage(
    crawler: any,
    choice: any,
    isSuccess: boolean,
    results: any,
    ...
    if (results.damage > 0) {
      message += ` but took ${results.damage} in the process`;
    }
  ...
  async moveToRoom(
    ...
    console.log(
      `Energy calculation: crawlerId=${crawlerId}, roomId=${connection.toRoomId}, previousVisits=${previousVisit.length}`,
    );
    if (previousVisit.length > 0) {
      ...
      console.log(
        `Energy cost reduced to ${energyCost} for previously visited room`,
      );
    } else {
      console.log(
        `First visit to room ${connection.toRoomId}, energy cost: ${energyCost}`,
      );
    }
    ...
    console.log(
      `Final energy cost: ${energyCost}, crawler current energy: ${crawler.energy}, debugEnergyDisabled: ${debugEnergyDisabled}`,
    );

    if (!debugEnergyDisabled && crawler.energy < energyCost) {
      return {
        success: false,
        error: `Not enough energy. Need ${energyCost}, have ${crawler.energy}`,
      };
    }
    ...
    if (!debugEnergyDisabled) {
      ...
      console.log(
        `Debug mode enabled: skipping energy deduction of ${energyCost}`,
      );
    }
  ...
  > {
    ...
    // If color can be null, provide a fallback (e.g., "#6B7280" for gray)
    return result.map((f) => ({
      id: f.id,
      name: f.name,
