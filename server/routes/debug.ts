...
export function registerDebugRoutes(app: Express) {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res.json({
          message: "Crawler healed - health, energy, and power restored",
          crawler: healedCrawler,
        });
      } catch (error) {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const { crawlers, crawlerPositions, crawlerEquipment } = await import(
          "@shared/schema"
        );
        ...
        res.json({
          message:
            "Successfully deleted " + userCrawlers.length + " crawler(s)",
  ...
  app.post(
    "/api/debug/clear-tactical-data/:crawlerId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        ...
        res.json({
          message:
            "Cleared tactical data for room " +
            currentRoom.id +
            " (" +
            currentRoom.name +
            ")",
  ...
  app.post(
    "/api/debug/clear-room-cache/:crawlerId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const crawlerId = parseInt(req.params.crawlerId);
        ...
        try {
          ...
          await storage.redisService.invalidateTacticalPositions(
            currentRoom.id,
          );
          ...
          res.json({
            success: true,
            message:
              "Cleared all cached data for room " +
              currentRoom.id +
              " (" +
              currentRoom.name +
              ")",
  ...
  app.delete(
    "/api/debug/clear-adjacent-cache/:crawlerId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const crawlerId = parseInt(req.params.crawlerId);
    ...
    async (req: any, res) => {
      try {
        ...
        res.json({
          ...
          message:
            "Redis fallback mode is currently " +
            (isFallbackMode ? "enabled" : "disabled"),
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        await storage.mobStorage.spawnSingleMob(
          currentRoom.id,
          roomData,
          true,
          true,
        ...
        console.log(
          "✅ Successfully spawned mob:",
          latestMob?.mob?.displayName,
        );
