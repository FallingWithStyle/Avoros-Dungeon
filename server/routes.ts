...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const availableDirections = await storage.getAvailableDirections(
          room.id,
        );
  ...
  app.get(
    "/api/crawlers/:id/explored-rooms",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const crawlerId = parseInt(req.params.id);
  ...
  app.get(
    "/api/season/secondary-sponsorships",
    isAuthenticated,
    async (req, res) => {
      try {
        const availableCrawlers =
          await storage.getAvailableSecondarySponsorships();
  ...
  app.post(
    "/api/crawlers/:id/restore-energy",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const crawlerId = parseInt(req.params.id);
  ...
  app.post(
    "/api/crawlers/:id/reset-position",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const crawlerId = parseInt(req.params.id);
  ...
  // Room state management routes
  app.get("/api/rooms/:roomId/state", async (req, res) => {
    try {
      const roomId = parseInt(req.params.roomId);
      let roomState = await storage.getRoomState(roomId);
      
      if (!roomState) {
        // Generate initial state if none exists
        const mobs = await storage.spawnMobsInRoom(roomId);
        roomState = await storage.createOrUpdateRoomState(roomId, {
          mobData: mobs,
          npcData: [],
          lootData: [],
          environmentData: {},
        ...
        // Update with current mob instances
        const activeMobs = await storage.getActiveMobsInRoom(roomId);
        roomState = { ...roomState, mobData: activeMobs };
      }
  ...
  app.post("/api/rooms/:roomId/state", isAuthenticated, async (req: any, res) => {
    try {
      const roomId = parseInt(req.params.roomId);
      const { mobData, npcData, lootData, environmentData } = req.body;
      
      const updatedState = await storage.createOrUpdateRoomState(roomId, {
        mobData,
        npcData,
        lootData,
        environmentData,
      ...
      // Also increment room activity
      await storage.incrementRoomActivity(roomId, 5);
  ...
  app.post("/api/rooms/:roomId/mob/:mobId/kill", isAuthenticated, async (req: any, res) => {
    try {
      const roomId = parseInt(req.params.roomId);
      const mobId = parseInt(req.params.mobId);
      
      await storage.killMob(mobId);
      await storage.incrementRoomActivity(roomId, 20); // Combat increases activity more
      
      // Check if room is now depleted
      const remainingMobs = await storage.getActiveMobsInRoom(roomId);
      if (remainingMobs.length === 0) {
        await storage.createOrUpdateRoomState(roomId, { isDepleted: true });
      }
      
      res.json({ success: true });
    } catch (error) {
      console.error("Error killing mob:", error);
      res.status(500).json({ message: "Failed to kill mob" });
    }
  });

  // Background process for mob respawning
  app.post("/api/admin/process-respawns", async (req, res) => {
    try {
      await storage.processRoomRespawns();
      res.json({ message: "Respawn processing complete" });
    } catch (error) {
      console.error("Error processing respawns:", error);
      res.status(500).json({ message: "Failed to process respawns" });
    }
  });
