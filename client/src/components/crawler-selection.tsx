    ...
    <div className="relative space-y-6 px-8 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[70vh] overflow-y-auto pr-2">
        {Array.isArray(candidates) && candidates.map((candidate: CrawlerCandidate) => {
          const predictedCrawlerId = nextCrawlerId + candidates.findIndex(c => c.id === candidate.id);
              ...
              <CardHeader className="pb-3 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <Avatar className="w-8 h-8">
                    <AvatarImage src={getAvatarUrl(candidate.name, predictedCrawlerId.toString())} alt={candidate.name} />
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                <Avatar className="w-10 h-10">
                  <AvatarImage src={getAvatarUrl(selectedCandidate.name, (nextCrawlerId + candidates.findIndex(c => c.id === selectedCandidate.id)).toString())} alt={selectedCandidate.name} />
                  <AvatarFallback className="bg-crawler text-white">
